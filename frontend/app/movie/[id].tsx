        ...
        <View style={s.headerRow}>
          ...
          <View style={{ flex: 1, paddingLeft: 12 }}>
            ...
            <View style={[s.statRow, { marginTop: 8 }]}>
              <View style={[s.studioPill, { borderColor: T.magenta }]}>
      ...
      {(m.colorBonus || 0) > 0 ? null : null}
        ...
        <View style={s.infoGrid}>
          ...
          <View style={[s.infoBtn, { flex: 1 }]}>
            ...
            <Text style={s.infoVal}>{franchise?.name || '—'}</Text>
          </View>
        </View>
        {/* Quick Actions */}
        {(() => {
        ...
        {m.crossoverFranchiseIds?.length ? (
          <View style={[s.infoBtn, { margin: 8 }]}>
            <Text style={[s.infoLabel, { color: T.magenta }]}>CROSSOVER</Text>
            <Text style={s.infoVal}>{m.crossoverFranchiseIds.map(fid => state.franchises.find(f => f.id === fid)?.name || '').filter(Boolean).join(', ')}</Text>
          </View>
        ) : null}
