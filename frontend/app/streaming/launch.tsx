        ...
        {tiers.map((t, idx) => (
          <View key={t.id} style={s.tierCard} testID={`tier-${idx}`}>
            ...
            <TouchableOpacity
              ...
              onPress={() => updateTier(idx, { isExclusive: !t.isExclusive })}
              testID={`tier-${idx}-exclusive`}
            >
              <MaterialCommunityIcons name={t.isExclusive ? 'star' : 'star-outline'} size={16} color={T.yellow} />
              <Text style={s.exclTxt}>Tier-exclusive content gating</Text>
            </TouchableOpacity>
          </View>
        ))}
