...
export function proposeBulkCatalogLicense(state: GameState, args: { fromRivalStudioId?: string; toRivalStudioId?: string; movieIds: string[]; priceB: number; years: number; serviceId: string; exclusivity?: boolean; dealKind?: 'catalog' | 'future_releases' | 'franchise_bulk'; franchiseId?: string; futureMovieCount?: number }): { state: GameState; offer?: BulkCatalogOffer; error?: string } {
  ...
  if (dealKind === 'franchise_bulk') {
    ...
    const fr = state.franchises.find(f => f.id === args.franchiseId);
    if (!fr) return { state, error: 'Franchise not found.' };
    if (fr.studioId !== targetId) return { state, error: 'Franchise belongs to a different studio.' };
  }
...
export function quoteFranchiseBulkValueB(state: GameState, franchiseId: string, years: number): number {
  const q = quoteFranchiseBulkLicense(state, { franchiseId, serviceId: '', years });
  if (q.error) return 0;
  return +(q.feeM / 1000).toFixed(3);
}
...
function finalizeBulkCatalog(state: GameState, offerId: string): { state: GameState; error?: string } {
  ...
  if (dealKind === 'future_releases' && o.serviceId && o.futureMovieCount) {
    ...
    const buyerName = buyerId === state.player.id ? state.player.name : state.rivals.find(r => r.id === buyerId)?.name || '—';
    const sellerName = sellerId === state.player.id ? state.player.name : state.rivals.find(r => r.id === sellerId)?.name || '—';
    const newsLog = [{ week: state.week, year: state.year, text: `📀 ${buyerName} bulk-licenses ${o.futureMovieCount} future ${sellerName} films for $${o.priceB.toFixed(2)}B / ${o.years}yr.` }, ...state.newsLog].slice(0, 100);
  ...
  if (dealKind === 'franchise_bulk' && o.serviceId && o.franchiseId) {
    ...
    if (svcIdx >= 0 && fr) {
      ...
      for (const mid of existingIds) {
        if (!svc.catalogMovieIds.includes(mid)) {
          licenseEntries.push({ movieId: mid, tierIds: [], feePaid: o.priceB * 1000 / Math.max(1, existingIds.length), yearsLicensed: o.years, expiresWeek: state.week, expiresYear });
        }
      }
      const newDeal = {
        id: uid('bld_'),
    ...
    const newsLog = [{ week: state.week, year: state.year, text: `📀 ${buyerName} bulk-licenses the entire ${frName} (${sellerName}) for $${o.priceB.toFixed(2)}B / ${o.years}yr.` }, ...state.newsLog].slice(0, 100);
...
export function counterOutboundBid(state: GameState, bidId: string, terms: { feeM?: number; royaltyPercent?: number; years?: number }): { state: GameState; error?: string } {
  ...
  if (richer <= 1.10) {
    ...
    news = [{ week: state.week, year: state.year, text: `🤝 ${licensor?.name || 'Agency'} accepts your terms — bid revised to $${newFee.toFixed(1)}M / ${newRoy.toFixed(1)}% / ${newYears}y.` }, ...state.newsLog].slice(0, 100);
  } else if (richer <= 1.30) {
    ...
    news = [{ week: state.week, year: state.year, text: `↩ ${licensor?.name || 'Agency'} counters: $${midFee.toFixed(1)}M / ${midRoy.toFixed(1)}% / ${midYears}y.` }, ...state.newsLog].slice(0, 100);
  } else {
    bids[idx] = { ...bid, status: 'rejected' };
    news = [{ week: state.week, year: state.year, text: `❌ ${licensor?.name || 'Agency'} walks away — your counter was too rich.` }, ...state.newsLog].slice(0, 100);
  }
